(function($) {
	$.widget("w.pagination", {
		...
		_create: function() {
			...
			$el.on("click", "li[data-role='paging']", $.proxy(function() {
				this.setNewPage();
		...
		_pagingStart: function() {
			var self 	= this,
				...
				if ( o.currentPage + o.pageLimit >= o.pageCount ) {
					 start = o.pageCount - (o.pageLimit * 2) - 1 ;
				} else {
					 start = (o.currentPage - o.pageLimit) <= 1 ? 1 : (o.currentPage - o.pageLimit) - 1;
				}
				return start;
		},
		_pagingEnd: function() {
		...
		_limitPaging: function() {
			...
			if(o.limitPaging) {
				var $paging = $("li[data-role='paging']", self.container).hide();
					$paging.slice(indexStart, indexEnd).css({ display: "inline"});

					$paging.filter("[data-page='1']").css({ display: "inline" });
					self.container.find("li[data-role='paging'][data-page='" + o.pageCount + "']").css({ display: "inline"});
				...
				$("li[data-role='paging']").css({ display: "inline" });
			}
		}
